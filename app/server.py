...
class NewsCategoryClassifier:
    def __init__(self, config: dict) -> None:
        ...
        sentence_transformer_embedding_dim = GLOBAL_CONFIG["model"]["featurizer"]["sentence_transformer_embedding_dim"]
        featurizer = TransformerFeaturizer(sentence_transformer_embedding_dim, sentence_transformer_model)
    ...
    def predict_proba(self, model_input: dict) -> dict:
        ...
        predictions_probablity = self.pipeline.predict_proba(model_input)[0]
        classes = self.classes



        return dict(zip(classes,predictions_probablity))
    ...
    def predict_label(self, model_input: dict) -> str:
        ...
        """
        predictions = self.pipeline.predict(model_input)[0]
...
def predict(request: PredictRequest):
    ...
    request_probablity = classifier.predict_proba(request.description)
    request_label = classifier.predict_label(request.description)
    
    log_data = {
        'timestamp':request_timestamp,
        'request':request,
        'prediction':request_probablity,
        'latency': datetime.datetime.now() - request_timestamp
    }

    log_file.write(str(log_data)+'\n')

    response = PredictResponse(scores=request_probablity,label=request_label)
